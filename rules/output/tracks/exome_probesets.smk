...
rule output_tracks_exome_probeset:
    ...
    shell:
        r"""
        cp bundled-data/track-enrichment-probesets/{wildcards.name}-targets-{wildcards.genome_release}.bed.gz {output.bed}
        cp bundled-data/track-enrichment-probesets/{wildcards.name}-targets-{wildcards.genome_release}.bed.gz.tbi {output.bed_tbi}
        """
