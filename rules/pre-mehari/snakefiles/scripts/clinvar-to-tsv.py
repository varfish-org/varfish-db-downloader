...
def run_conversion(
    ...
    with inputf, open(path_tsv, "wt") as outputf:
        ...
        for line in inputf:
            ...
            if clinvar_record:
                clinvar_record.print_for_postgres(outputf)
...
if __name__ == "__main__":
    with open(snakemake.input.tsv, "rt") as inputf:
        full_set_tsv = inputf.read()
    hgnc_map = full_set_to_hgnc_map(full_set_tsv)
    clinvar_release = snakemake.params.version
    genomebuild = snakemake.wildcards.genomebuild
    run_conversion(hgnc_map, clinvar_release, genomebuild, snakemake.input.jsonl, snakemake.output.tsv)
    with open(snakemake.output.release_info, 'w') as f:
        f.write("\t".join(["table", "version", "genomebuild", "null_value"]) + "\n")
        f.write("\t".join(["Clinvar", clinvar_release, genomebuild, ""]) + "\n")